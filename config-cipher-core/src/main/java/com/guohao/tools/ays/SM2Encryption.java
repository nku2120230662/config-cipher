...
// SM2 + SM4-GCM 混合加密实现
public class SM2Encryption implements EncryptionAlgorithm {
    private static final String SM2_ALGORITHM = "EC";
    private static final String SM2_CIPHER = "SM2";
    ...
    public SM2Encryption() throws Exception {
        generateKeyPair();
    }

    private void generateKeyPair() throws Exception {
        BouncyCastleSupport.ensureProvider();
        KeyPairGenerator keyGen = KeyPairGenerator.getInstance(SM2_ALGORITHM, "BC");
    ...
    public void encrypt(java.io.InputStream input, java.io.OutputStream output, String key) throws Exception {
        ...
        if (material.publicKey == null) {
            throw new IllegalArgumentException("SM2 public key is required for encryption");
        }

        byte[] sm4Key = new byte[SM4_KEY_BYTES];
        byte[] iv = new byte[IV_LENGTH];
        SecureRandom random = new SecureRandom();
        random.nextBytes(sm4Key);
        random.nextBytes(iv);

        byte[] encKey = encryptKey(sm4Key, material.publicKey);
        ...
        cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(sm4Key, "SM4"), new GCMParameterSpec(TAG_BITS, iv));
    ...
    public void decrypt(java.io.InputStream input, java.io.OutputStream output, String key) throws Exception {
        ...
        if (material.privateKey == null) {
            throw new IllegalArgumentException("SM2 private key is required for decryption");
        }

        DataInputStream in = new DataInputStream(input);
        int version = in.readUnsignedByte();
        if (version != VERSION_V1 && version != VERSION_V2) {
            throw new IllegalArgumentException("Unsupported SM2 payload version: " + version);
        }
        ...
        byte[] sm4Key = decryptKey(encKey, material.privateKey);
        BouncyCastleSupport.ensureProvider();
        Cipher cipher = Cipher.getInstance(SM4_TRANSFORMATION, "BC");
        cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(sm4Key, "SM4"), new GCMParameterSpec(TAG_BITS, iv));
    ...
    private byte[] encryptKey(byte[] key, PublicKey publicKey) throws Exception {
        BouncyCastleSupport.ensureProvider();
        Cipher cipher = Cipher.getInstance(SM2_CIPHER, "BC");
    ...
    private KeyMaterial resolveKeyMaterial(String key) throws Exception {
        if (key == null || key.trim().isEmpty()) {
            throw new IllegalArgumentException(
                    "SM2 key material is required. Provide 'pub:BASE64' and/or 'priv:BASE64' (separated by ';').");
        }
        ...
        for (String part : parts) {
            ...
            if (trimmed.startsWith("pub:")) {
                ...
                try {
                    ...
                    throw new IllegalArgumentException(
                            "Unable to parse SM2 key. Use 'pub:BASE64' or 'priv:BASE64'. "
    ...
    private PublicKey decodePublicKey(String base64) throws Exception {
        ...
        KeyFactory factory = KeyFactory.getInstance(SM2_ALGORITHM, "BC");
